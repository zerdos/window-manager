...
export class Snap {
  ...
  moveEnd(win) {
    ...
    if (this.horizontal.style.display === "block") {
      ...
      const adjust = win.minimized
        ? (win.height - win.height * win.minimized.scaleY) / 2
        : 0;
      switch (this.horizontal.side) {
        case "top":
          win.y = this.horizontal.y - adjust + spacing +
            this.options.indicator / 2;
          break;

        case "bottom":
          win.bottom = Math.floor(
            this.horizontal.y + adjust - spacing + this.options.indicator / 2,
          );
          break;
      }
      win.attachToScreen(
    ...
    if (this.vertical.style.display === "block") {
      ...
      const adjust = win.minimized
        ? (win.width - win.width * win.minimized.scaleX) / 2
        : 0;
      switch (this.vertical.side) {
        ...
        case "right":
          win.right = Math.floor(
            this.vertical.x + adjust - spacing + this.options.indicator / 2,
          );
          break;
      }
      win.attachToScreen(
